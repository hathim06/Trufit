...
const getDeliveredDate = (order, item = null) => {
    const deliveredAt = item?.deliveredAt || order?.deliveredAt || order?.updatedAt;
...
const canReturnOrderItem = (order, item) => {
    if (!order || !item || order.orderStatus !== 'Delivered') return false;
    if (item.status === 'Cancelled' || item.status === 'Return Pending' || item.status === 'Returned') return false;
    const deliveredAt = getDeliveredDate(order, item);
    if (!deliveredAt || Number.isNaN(deliveredAt.getTime())) return false;
    return new Date() <= new Date(deliveredAt.getTime() + RETURN_WINDOW_MS);
};
...
const registerCouponUsage = async (coupon, userId, orderId, session) => {
    ...
    if (Number(coupon.usageCount) >= Number(coupon.maxUsage || 0)) {
...
const createOrder = async (userId, addressId, paymentMethod, couponCode = null, useWallet = false) => {
    ...
    try {
        ...
        if (couponCode) {
            ...
            discountAmount = calculateCouponDiscount(couponObj, subtotal);
...
const finalizeOnlineOrder = async (orderId, userId) => {
    ...
    try {
        ...
        if (!order.items.some(item => item.status !== 'Cancelled')) {
            throw new Error('There are no active items left in this order to confirm');
        }
        ...
        order.failedPaymentExpiresAt = null;
...
const returnOrder = async (orderId, userId, reason) => {
    ...
    if (!canReturnOrder(order)) {
        throw new Error('Return window expired. Orders can be returned within 7 days of delivery.');
    }
...
const returnOrderItem = async (orderId, itemId, userId, reason) => {
    ...
    if (!canReturnOrderItem(order, item)) {
        throw new Error('Return window expired. Items can be returned within 7 days of delivery.');
    }
